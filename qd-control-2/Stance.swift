...
struct Stance: CustomStringConvertible {
	...
	func newStance(side: Side, translation: simd_float4, rotation: Float)->Stance {
		var targets: [simd_float4] = targets
		let halfTranslation = translation * 0.5
		switch side {
			case .frbl:
				targets[0] += halfTranslation
				targets[1] -= halfTranslation
				targets[2] -= halfTranslation
				targets[3] += halfTranslation
			case .flbr:
				targets[0] -= halfTranslation
				targets[1] += halfTranslation
				targets[2] += halfTranslation
		...
		return Stance(targets, centroid: centroid, tangent: tangent, orthogonal: orthogonal, progress: 0.0, stepSize: 0.0)
	}
}
